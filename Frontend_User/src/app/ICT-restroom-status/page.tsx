...
import { getRestroomStatuses } from '@/app/services/api';
...
const baseFloorRestroomSpots: { [key: string]: RestroomSpot[] } = {
  'ชั้น 1': [
    { name: 'ห้องน้ำหญิง / ชั้น 1 โซน B', cx: 260, cy: 227, status: 'available' },
    { name: 'ห้องน้ำชาย / ชั้น 1 โซน B', cx: 253, cy: 247, status: 'available' },
    { name: 'ห้องน้ำหญิง / ชั้น 1 โซน A', cx: 769, cy: 164, status: 'available' },
    { name: 'ห้องน้ำชาย / ชั้น 1 โซน A', cx: 777, cy: 184, status: 'available' },
  ],
  'ชั้น 2': [
    { name: 'ห้องน้ำหญิง / ชั้น 2 โซน หอประชุมพะเยา', cx: 320, cy: 220, status: 'available' },
    { name: 'ห้องน้ำชาย / ชั้น 2 โซน หอประชุมพะเยา', cx: 312, cy: 241, status: 'available' },
    { name: 'ห้องน้ำหญิง / ชั้น 2 โซน D (บริเวณ งานบริการระบบเครือข่ายคอมพิวเตอร์)', cx: 575, cy: 270, status: 'available' },
    { name: 'ห้องน้ำชาย / ชั้น 2 โซน D (บริเวณ งานบริการระบบเครือข่ายคอมพิวเตอร์)', cx: 580, cy: 290, status: 'available' },
    { name: 'ห้องน้ำหญิง / ชั้น 2 โซน A', cx: 735, cy: 405, status: 'available' },
    { name: 'ห้องน้ำชาย / ชั้น 2 โซน A', cx: 744, cy: 425, status: 'available' },
    { name: 'ห้องน้ำหญิง / ชั้น 2 โซน B', cx: 295, cy: 460, status: 'available' },
    { name: 'ห้องน้ำชาย / ชั้น 2 โซน B', cx: 289, cy: 480, status: 'available' },
  ],
  'ชั้น 3': [
    { name: 'ห้องน้ำหญิง / ชั้น 3 โซน C (ห้องน้ำชำรุดใช้งานไม่ได้)', cx: 305, cy: 224, status: 'maintenance' },
    { name: 'ห้องน้ำชาย / ชั้น 3 โซน C (ห้องน้ำชำรุดใช้งานไม่ได้)', cx: 299, cy: 244, status: 'maintenance' },
    { name: 'ห้องน้ำหญิง / ชั้น 3 โซน D (บริเวณห้องCITCOMS)', cx: 645, cy: 295, status: 'available' },
    { name: 'ห้องน้ำชาย / ชั้น 3 โซน D (บริเวณห้องCITCOMS)', cx: 653, cy: 315, status: 'available' },
    { name: 'ห้องน้ำหญิง / ชั้น 3 โซน A', cx: 775, cy: 440, status: 'available' },
    { name: 'ห้องน้ำชาย / ชั้น 3 โซน A', cx: 783, cy: 460, status: 'available' },
    { name: 'ห้องน้ำหญิง / ชั้น 3 โซน B', cx: 363, cy: 490, status: 'available' },
    { name: 'ห้องน้ำชาย / ชั้น 3 โซน B', cx: 356, cy: 510, status: 'available' },
  ],
  'ชั้น 4': [
    { name: 'ห้องน้ำหญิง / ชั้น 4 โซน B', cx: 275, cy: 387, status: 'available' },
    { name: 'ห้องน้ำชาย / ชั้น 4 โซน B', cx: 268, cy: 407, status: 'available' },
    { name: 'ห้องน้ำหญิง / ชั้น 4 โซน A', cx: 780, cy: 325, status: 'available' },
    { name: 'ห้องน้ำชาย / ชั้น 4 โซน A', cx: 786, cy: 344, status: 'available' },
  ],
};
...
export default function ICTRestroomStatusPage() {
  ...
  const [spotsData, setSpotsData] = useState<{ [key: string]: RestroomSpot[] }>(baseFloorRestroomSpots);
  ...
  /**
   * ดึงข้อมูลสถานะห้องน้ำจากฐานข้อมูล Backend (/api/restrooms) และแม็พเข้ากับจุดสี SVG
   */
  const fetchDbRestroomStatuses = useCallback(async () => {
    try {
      setLoading(true);
      const res = await getRestroomStatuses();

      if (res && res.success && Array.isArray(res.data)) {
        const dbItems: any[] = res.data;

        const updatedSpots: { [key: string]: RestroomSpot[] } = {};

        Object.keys(baseFloorRestroomSpots).forEach((floorKey) => {
          const floorNum = floorKey.replace(/\D/g, '').trim();

          updatedSpots[floorKey] = baseFloorRestroomSpots[floorKey].map((spot) => {
            // หา Record ใน DB ที่ตรงกับชั้น, เพศ, และโซน
            const match = dbItems.find((dbItem) => {
              const dbFloor = String(dbItem.floor_level || '').trim();
              if (dbFloor !== floorNum) return false;

              const dbLoc = dbItem.location_name || '';
              const isDbFemale = dbLoc.includes('หญิง');
              const isSpotFemale = spot.name.includes('หญิง');
              if (isDbFemale !== isSpotFemale) return false;

              if (spot.name.includes('โซน A') && dbLoc.includes('โซน A')) return true;
              if (spot.name.includes('โซน B') && dbLoc.includes('โซน B')) return true;
              if (spot.name.includes('โซน C') && (dbLoc.includes('โซน C') || dbLoc.includes('หุ่นยนต์'))) return true;
              if (spot.name.includes('โซน D') && (dbLoc.includes('โซน D') || dbLoc.includes('CITCOMS') || dbLoc.includes('เครือข่าย'))) return true;
              if (spot.name.includes('หอประชุม') && dbLoc.includes('หอประชุม')) return true;

              return false;
            });

            if (match) {
              const isAvailable = match.status === 'พร้อมใช้งาน';
              return {
                ...spot,
                status: isAvailable ? 'available' : 'maintenance',
                reason: !isAvailable ? (match.remark || 'ปิดปรับปรุงชั่วคราว') : undefined,
              };
            }
        ...
        setSpotsData(updatedSpots);
        setLastUpdated(new Date().toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit', second: '2-digit' }));
      }
    } catch (err) {
      console.warn('⚠️ ไม่สามารถดึงสถานะห้องน้ำจาก DB ได้ (ใช้ค่าเริ่มต้น):', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    fetchDbRestroomStatuses();
    // อัปเดตสถานะอัตโนมัติทุก 15 วินาที
    const interval = setInterval(fetchDbRestroomStatuses, 15000);
  ...
  const currentSpots = spotsData[currentFloorKey] || baseFloorRestroomSpots[currentFloorKey] || [];
  ...
  return (
    <div className="min-h-screen bg-[#FDF9FF] flex flex-col font-sans pb-10">
      ...
      <div className="w-full bg-[#E4C5F9] text-black px-4 py-4 md:px-8 md:py-5 flex items-center justify-between shadow-sm mb-6">
        ...
        <button
          onClick={fetchDbRestroomStatuses}
          disabled={loading}
          className="text-xs bg-white text-[#6610A8] font-bold px-3 py-1.5 rounded-xl border border-[#6610A8]/30 hover:bg-purple-50 active:scale-95 transition-all flex items-center gap-1 shadow-sm"
        >
          <span>{loading ? '⏳' : '🔄'}</span>
          <span className="hidden sm:inline">รีเฟรชสถานะ</span>
        </button>
      </div>
      ...
      <div className="w-full max-w-4xl mx-auto p-4 md:p-8 flex flex-col gap-6 pt-0">
            ...
            {hasSelected ? (
                ...
                <svg viewBox="0 0 1000 700" className="w-full h-auto block rounded-2xl select-none overflow-visible">
                  ...
                  {currentSpots.map((spot, index) => {
                    ...
                    return (
                      ...
                      >
                        {/* พื้นที่กดโปร่งใส ขยายให้กดง่าย */}
                        <circle cx={spot.cx} cy={spot.cy} r={25} fill="transparent" />

                        {/* จุดแสดงผลจริง: เขียว = available, แดง = maintenance */}
                        <circle
                          cx={spot.cx}
                          cy={spot.cy}
                          r={10}
                          className={`${
                            spot.status === 'available'
                              ...
                              ? 'fill-[#E00000]'
                              : 'fill-[#9CA3AF]'
                          } ${isSelected ? 'animate-pulse stroke-4 stroke-yellow-300' : ''} transition-all duration-200 shadow-md`}
                        />
                      </g>
                    );
                  })}
              ...
              <div className="flex flex-col items-center justify-center gap-2 text-black py-16">
                ...
                >
                  ...
                  <path d="M15 6v15" />
                  <path
                    d="M12 4a2.5 2.5 0 0 0-2.5 2.5c0 1.8 2.5 4.5 2.5 4.5s2.5-2.7 2.5-4.5A2.5 2.5 0 0 0 12 4z"
                    strokeWidth="1"
                  />
                  <circle cx="12" cy="6.5" r="0.8" fill="black" />
                </svg>
                <p className="text-sm font-semibold text-gray-500">กรุณาเลือกห้องน้ำหรือชั้นด้านล่างเพื่อดูแผนที่</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white border border-black/10 rounded-2xl p-4 shadow-sm flex items-center justify-around">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs md:text-sm font-bold text-black">ไม่พร้อมใช้งาน (ปิดปรับปรุง)</span>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div
          className={`bg-white border border-black/30 rounded-2xl p-4 text-center font-bold text-sm md:text-base shadow-sm ${
                    ...
                    {activeFloor === floor && (
                      <div className="bg-white flex flex-col">
                        {locationHierarchy[floor].map((subItem, index) => {
                          const currentSpotItem = (spotsData[floor] || []).find((s) => s.name === subItem);
                          const isUnavailable = currentSpotItem?.status === 'maintenance';
                          ...
                          return (
                            <div
                              ...
                              onClick={() => {
                                ...
                                setActiveFloor(null);
                                setActivePopup(subItem);
                              }}
                              className="px-6 py-2.5 hover:bg-purple-50 cursor-pointer text-xs md:text-sm text-gray-700 border-b border-gray-50 last:border-none flex items-center justify-between"
                            >
                              <span>- {subItem}</span>
                              <span
                              ...
                              >
                                {isUnavailable ? 'ไม่พร้อมใช้งาน' : 'พร้อมใช้งาน'}
                              </span>
                            </div>
                          );
                        })}
