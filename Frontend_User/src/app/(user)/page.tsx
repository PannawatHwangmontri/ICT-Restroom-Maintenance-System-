...
import { initLiff, getLiffProfile, LiffProfile } from '@/lib/liff';
import { getAllRequests } from '@/app/services/api';
...
export default function Home() {
  ...
  useEffect(() => {
    const initApp = async () => {
      try {
        ...
        // ดึงจำนวนเรื่องที่กำลังดำเนินการจาก Backend
        try {
          const res = await getAllRequests();
          if (res && res.data) {
            const activeRequests = res.data.filter(
              (r) => r.status === 'รอรับเรื่อง' || r.status === 'กำลังดำเนินการ' || r.status === 'แจ้งแล้ว'
            );
            setPendingCount(activeRequests.length);
          }
        } catch {
          // หาก Backend ยังไม่รันก็ให้ข้ามได้
        }
      } catch (error) {
  ...
  if (!isReady) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FDF9FF] text-purple-900 font-bold">
        กำลังโหลดระบบ...
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FDF9FF] flex flex-col font-sans">
      <div className="w-full max-w-4xl mx-auto p-4 md:p-8 flex flex-col flex-1">
        ...
        <div className="bg-white border-[2px] border-[#6610A8] rounded-2xl p-4 md:p-6 shadow-sm flex items-center space-x-4 mb-6">
          <div className="w-8 h-8 rounded-full overflow-hidden shrink-0 flex items-center justify-center bg-purple-100">
            {profile?.pictureUrl ? (
              <img src={profile.pictureUrl} alt="User Avatar" className="w-full h-full object-cover" />
            ) : (
              <img src="/photo/Hello.png" alt="Hello Icon" className="w-5 h-5 object-contain" />
            )}
          </div>
          <div>
        ...
        <div className="flex flex-col flex-1 justify-end">
            ...
            <div className="overflow-hidden grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
              ...
              <Link href="/report" className="w-full flex">
                <button className="w-full border-[2px] border-[#6610A8] rounded-2xl p-5 bg-white shadow-sm flex flex-col items-start text-left active:scale-95 transition-transform hover:bg-purple-50 justify-between h-full">
                  ...
                  <span className="text-xs font-bold text-[#6610A8] mt-4 flex items-center">
                    คลิกเพื่อแจ้งซ่อม &rarr;
                  </span>
                </button>
              </Link>
              ...
              <Link href="/status" className="w-full flex">
                <button className="w-full border-[2px] border-[#6610A8] rounded-2xl p-5 bg-white shadow-sm flex flex-col items-start text-left relative active:scale-95 transition-transform hover:bg-purple-50 justify-between h-full">
                  ...
                  <span className="text-xs font-bold text-[#6610A8] mt-4">
                    {pendingCount > 0 ? `${pendingCount} เรื่องกำลังดำเนินการ &rarr;` : 'ดูรายการทั้งหมด &rarr;'}
                  </span>
                </button>
              </Link>

              {/* ปุ่มที่ 3: สถานะห้องน้ำทั้งหมดในคณะ */}
          ...
          <div
            className="bg-[#E4C5F9] p-3 text-center cursor-pointer flex justify-center items-center gap-2 rounded-xl shadow-sm hover:bg-[#d8b0f7] transition-colors select-none"
